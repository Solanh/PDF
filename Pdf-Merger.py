...
log_dir = os.path.join(os.path.expanduser("~"), "Documents", "PDFMerger")  # Store logs in Documents
os.makedirs(log_dir, exist_ok=True)  # Ensure directory exists

LOG_FILE = os.path.join(log_dir, "pdf_merger.log")  # Log file path
logging.basicConfig(
    filename=LOG_FILE,
...
# === Detect and Set Poppler Path Dynamically ===
STORED_POPPLER_PATH = os.environ.get("POPPER_PATH", "")  # Allow users to set via environment variable

if getattr(sys, 'frozen', False):  # Running as PyInstaller exe
    BASE_DIR = sys._MEIPASS  # PyInstaller temp dir
    poppler_path = os.path.join(BASE_DIR, "poppler-24.08.0", "Library", "bin")
elif os.path.exists(STORED_POPPLER_PATH):  # Use env variable if set
    poppler_path = STORED_POPPLER_PATH
else:
    poppler_path = None  # No valid path found

if getattr(sys, 'frozen', False):  #Running as PyInstaller exe
    BASE_DIR = sys._MEIPASS  # PyInstaller temp dir
    poppler_path = os.path.join(BASE_DIR, "poppler-24.08.0", "Library", "bin")
else:  #Running as standard Python script
    poppler_path = STORED_POPPLER_PATH  #Use manually installed Poppler path
...
class FileUploader:
    ...
    def ending_screen(self):
        ...
        self.back_to_main = tk.Button(self.end_frame, text="Back to main menu", command=self.setup_ui, width=20)
        self.back_to_main.grid(row=3, column=0, sticky="n", pady=(0, 10))   
        ...
        try:
            for i in self.new_order:
                if i in self.pages_pdf:
                    writer.add_page(self.pages_pdf[i])  #Always a PageObject 
                else:
            ...
            logging.info(f"PDF successfully saved as {abs_path}")
    ...
    def preview_pdf(self, counter):
        ...
        try:
            if counter == 0:
                for i in self.new_order:
                    logging.info(f"Generating preview for page {i}")
                ...
                for i in range(1, len(self.new_order) + 1):
                    if i not in self.pages_img:
                        logging.warning(f"Warning: Page {i} preview not found.")
                        continue
            ...
            logging.warning(f"Error while generating preview: {e}")
    ...
    def process_uploads(self):
        ...
        for filename, filepath in self.files.items():
            if filepath.endswith(".pdf"):
                ...
                images = convert_from_path(filepath, dpi=100, poppler_path=poppler_path)
                for img in images:
